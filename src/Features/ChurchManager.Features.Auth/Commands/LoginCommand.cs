 ...
 * ----  LOGOUT COMMAND ------
 */
public record LogoutCommand([Required] string Username) : IRequest;

public class LogoutHandler(IUserLoginDbRepository dbRepository) : IRequestHandler<LogoutCommand>
{
    public async Task Handle(LogoutCommand request, CancellationToken ct)
    {
        await dbRepository.LogoutUserAsync(request.Username.AsGuid(), ct);
    }
}
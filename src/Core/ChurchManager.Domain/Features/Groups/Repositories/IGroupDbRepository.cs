...
namespace ChurchManager.Domain.Features.Groups.Repositories
{
    public interface IGroupDbRepository : IGenericDbRepository<Group>
    {
        ...
        Task<IEnumerable<GroupViewModel>> GroupsWithChildrenAsync(int? groupTypeId = null,int? churchId = null, int? parentGroupId = null, int maxDepth = 10, CancellationToken ct = default);
        Task<IEnumerable<GroupViewModel>> GroupWithChildrenAsync(int groupId, int maxDepth = 10,  CancellationToken ct = default);
        Task<int> GroupMembersCountAsync(int groupId, bool includeLeaders = false, CancellationToken ct = default);
